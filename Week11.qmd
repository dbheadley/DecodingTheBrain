...
## Spike counts as a Poisson point process
...
ax.plot(x, gmm.weights_[0]*norm.pdf(x, means[0], np.sqrt(covs[0])), label='Tuned')
ax.plot(x, gmm.weights_[1]*norm.pdf(x, means[1], np.sqrt(covs[1])), label='Untuned')
ax.legend()
```

Here we got two components that match our intuition about there being a narrow, tuned, distribution on the high end (blue) and a broad untuned distribution (orange) on the low end. Just below $R^2=0.8$ the distributions intersect. This is where the threshold should be for declaring a unit tuned. To find that numerically, we can subtract the tuned curve from the untuned curve and find the point where that difference goes from positive to negative.
