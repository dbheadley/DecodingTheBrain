...
for ind1, comp1 in enumerate(comp_list):
    for ind2, comp2 in enumerate(comp_list):
        if ind1==ind2:
            ax[ind1,ind2].hist(pca_pool[:,comp1], bins=50)
            ax[ind1,ind2].set_title('Component {x}'.format(x=comp1))
        elif ind1>ind2:
            ax[ind1,ind2].axis('off')
        else:
            ax[ind1,ind2].scatter(pca_pool[:,comp1],pca_pool[:,comp2], alpha=1, s=.001)
            ax[ind1,ind2].set_title('Components {x} vs {y}'.format(x=comp1,y=comp2))
